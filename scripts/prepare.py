...
with ZipFile(sm.input[0]) as zip_file:

    for year in range(2005, 2023):

        with zip_file.open(f"epi_cpsbasic_{year}.dta") as dta_file:
...
df = pd.concat(dfs, ignore_index=True, copy=False).set_index(["hhid", "year", "month"])

df.to_parquet(sm.output[0])
